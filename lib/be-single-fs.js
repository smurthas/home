...
function getPermissions(object, grantIDs) {
  ...
  var objectGrants = object._grants || (object.meta && object.meta._grants);
...
module.exports.upsertCollection = function(options, callback) {
  ...
  } else if (!permissions.modifyAttributes) {
    return callback({
      unauthorized: 'modify'
    });
  }
  ...
  console.error('collection.meta before', collection.meta);
  console.error('operation', operation);

  fiddle(operation, null, collection.meta);
  console.error('collection.meta after', collection.meta);
